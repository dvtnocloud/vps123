...
# websocat (thay websockify, không cần python)
ADD https://github.com/vi/websocat/releases/latest/download/websocat_x86_64-linux /usr/local/bin/websocat
RUN chmod +x /usr/local/bin/websocat

# Start noVNC WebSocket gateway
...
RUN echo '#!/bin/sh' > /tunnel.sh && \
    echo 'echo "[OK] Tạo Cloudflare Tunnel..."' >> /tunnel.sh && \
